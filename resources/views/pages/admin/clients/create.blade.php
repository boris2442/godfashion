...
<div class="min-h-screen bg-gray-50 dark:bg-neutral-900 py-10">
    <div class="max-w-2xl mx-auto px-4">
        <div
            class="bg-white dark:bg-neutral-800 border border-gray-200/70 dark:border-neutral-700 rounded-2xl shadow-lg">
            <div class="p-6 sm:p-8">
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Créer un client</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-neutral-300">
                    Renseignez les informations du client ci-dessous.
                </p>

                <form action="{{ route('admin.clients.store') }}" method="POST" class="mt-8 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="number" id="phone" name="phone" value="{{ old('phone') }}" inputmode="tel"
                            class="mt-1 w-full rounded-xl border border-gray-300 dark:border-neutral-600 bg-white dark:bg-neutral-900 px-4 py-2.5 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Ex: 690123456" required>
                        @error('phone')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="adresse" name="adresse" rows="3"
                            class="mt-1 w-full rounded-xl border border-gray-300 dark:border-neutral-600 bg-white dark:bg-neutral-900 px-4 py-2.5 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="Ex: Yaoundé, Omnisports">{{ old('adresse') }}</textarea>
                        @error('adresse')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
